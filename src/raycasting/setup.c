...
/*   Updated: 2023/03/28 15:23:58 by jvan-tol      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
bool	setup(t_data *data)
{
	...
	if (!data->mlx)
		return (ft_error("failed initializing mlx", false));
	data->background = mlx_new_image(data->mlx, WIDTH, HEIGHT);
	if (!data->background)
		return (ft_error("failed creating background image", false));
	data->foreground = mlx_new_image(data->mlx, WIDTH, HEIGHT);
	if (!data->foreground)
		return (ft_error("failed creating foreground image", false));
